...
- **Управление сессией:** `Старт` запускает новую сессию, а после `Пауза` возобновляет текущую без сброса прогресса; `Сброс` обнуляет счётчик и фазу текущего упражнения.
- **Пауза и продолжение:** при паузе обработка кадров останавливается, в сцене показывается состояние «Упражнение приостановлено», затем `Старт` продолжает ту же сессию.
