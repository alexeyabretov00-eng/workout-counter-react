...
- **Страницы** подключают модуль как композицию; локальные `pages/<Page>/components` используются только для склейки модулей на экране (если страница не сведена к одному импорту модуля). Экран тренировки: оркестрация в **`HomeModule.tsx`** и слои **`HomeModule`** (`contexts`, `selectors`, `containers`, `hooks`, …) — в [docs/src-layout.md](src-layout.md); маршрут остаётся в `src/pages/HomePage/`.
