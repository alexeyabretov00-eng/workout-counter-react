...
- **Управление сессией:** одна кнопка переключается между `Старт` и `Пауза`: первый запуск и возобновление после паузы — это `Старт`, во время активного выполнения та же кнопка становится `Пауза`. `Сброс` обнуляет счётчик и фазу текущего упражнения. Пока идёт таймер отдыха после «Стоп камера», кнопки `Сброс` и `Стоп камера` неактивны.
...
- **Остановка камеры:** `Стоп камера` завершает сессию, отключает камеру и запускает таймер отдыха на выбранное время.
...
- **Поведение:** по нажатию «Стоп камера» сессия останавливается, камера отключается, на пустом canvas показывается обратный отсчёт с кольцом прогресса. Во время этого отсчёта кнопки «Сброс» и «Стоп камера» отключены до окончания отдыха или нажатия «Старт».
