...
Для **каждой фичи** заводите в репозитории пару артефактов (сначала план и чеклист, затем реализация в коде):
...
**Коммиты по фиче (три штуки):** (1) только план и чеклист в `features/<имя>/`; (2) реализация (код, тесты, документация по чеклисту), без архивации; (3) только перенос каталога фичи в `features/_archive/<то же имя>/` и правка относительных путей в её markdown. Не объединяйте эти этапы в одном коммите.
