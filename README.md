...
- **Выбор упражнения:** выпадающий список и голосовые синонимы приходят из API (`/api/exercises`) и хранятся в Redux (`home.exerciseCatalogEntries`). В ходе активной сессии список блокируется.
...
- **Несколько страниц (React Router):** публичные маршруты **«Вход»** (`/login`) и **«Регистрация»** (`/register`); после входа — защищённые маршруты под **`RequireAuth`**. Навигация и доступ зависят от роли: `superadmin` — все страницы, `admin` — все кроме `/admin/users`, `user` — только `/home` и `/history`. Страница `/admin/users` предназначена для управления пользователями и доступна только `superadmin`; если для пользователя выставлен флаг `mustChangePassword`, приложение принудительно ведёт на `/change-password` до смены пароля. Главная тренировки — путь **`/home`**; полная логика (камера, сессия, голос) только в **`HomeModule`** на этой странице. Остальные экраны — без сетки тренировки.
...
- После существенных правок в конкретном модуле (`src/modules/<ModuleName>/`) запускайте точечную CLI-проверку: `npx eslint "src/modules/<ModuleName>/**/*.{ts,tsx}"`; при автоисправляемых ошибках — `npx eslint "src/modules/<ModuleName>/**/*.{ts,tsx}" --fix` и повторная проверка. IDE-диагностика может запаздывать, поэтому финальная проверка — через CLI.
...
**Этап 4 (интеграция после трёх коммитов):** push по необходимости, создание PR, merge PR (без автоматического удаления рабочей ветки), затем локальный переход на основную ветку.
...
Распознавание непрерывное; между одинаковыми командами действует короткая задержка (антидребезг). Команды смены упражнения строятся из каталога упражнений API (поля `name` и `voiceAliases` записи упражнения).
...
2. Реализуйте интерфейс `ExerciseDetector` из `src/modules/HomeModule/exercises/types.ts`:
   - `id` (должен совпадать с `detectorKey` в записи упражнения API);
...
4. Добавьте запись упражнения в БД/админке (`slug`, `name`, `description`, `detectorKey`, `voiceAliases`, `sortOrder`, `isActive`), чтобы упражнение попало в UI и голосовой выбор.
5. Добавьте или расширьте тесты в `src/modules/HomeModule/exercises/__tests__/detectors.test.ts` (или отдельном файле рядом), чтобы зафиксировать пороги и сценарии счёта.
