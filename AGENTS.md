...
7. **Слои главной (`modules/HomeModule/logic`, `contexts`, `selectors`, `containers`):** подпапки по соглашению, баррели `index.ts`, импорты между папками `src` — из баррелей. [.cursor/rules/src-layout.mdc](.cursor/rules/src-layout.mdc), [docs/src-layout.md](docs/src-layout.md).
