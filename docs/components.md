...
- `src/components` — **переиспользуемые** презентационные блоки (примитивы вроде кнопки, селекта, бейджа), которые имеет смысл импортировать с разных экранов.
- Виджеты, относящиеся к **одной странице** или к **оболочке приложения**, держите рядом с ними: например `src/modules/HomeModule/components/` (как `HomeLayout` и блоки тренировки) или `src/App/components/` (как навигация). Структура папки на компонент и `*.styled.tsx` — те же, что ниже.
...
Примеры в репозитории: `src/components/Select/`, `src/components/Button/`, `src/components/Badge/`.

## Объявление компонента (функция)

- Пишите **презентационный** компонент как **именованный** `const` со стрелочной функцией и аннотацией **`React.FC<…>`**:
  `export const MyWidget: React.FC<MyWidgetProps> = ({ … }) => { … }`.
  Так явно зафиксирован контракт пропсов, имя остаётся в стеке и в React DevTools без лишнего кода.
  Отдельный `import React` для `React.FC` не обязателен, если TypeScript уже подхватывает глобальный namespace `React` (как в остальных TSX файлах проекта).
...
- Компоненты **`logic`** и **`containers`** (оркестрация, слоты с селекторами) оформляйте как **именованный** `const` со стрелочной функцией; **`React.FC`** для них **не обязателен** — см. [docs/src-layout.md](src-layout.md).
...
Импорт из барреля **`src/components`** (между папками верхнего уровня `src/*` — не из файла компонента напрямую). Относительный путь зависит от места файла; например из `src/App/App.tsx`: `import { Button, Select } from '../components'`; из `src/modules/HomeModule/HomeModule.tsx`: `import { … } from '@components'`. Локальные виджеты экрана тренировки импортируйте из барреля **`./components`** внутри **`HomeModule`** (или из **`./components`** под `App` для оболочки).
...
Оркестрация экрана тренировки, контексты, селекторы и контейнеры слотов главной — под `src/modules/HomeModule/`; см. [docs/src-layout.md](src-layout.md).
