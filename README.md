...
- **Управление сессией:** одна кнопка переключается между `Старт` и `Пауза`: первый запуск и возобновление после паузы — это `Старт`, во время активного выполнения та же кнопка становится `Пауза`. `Сброс` обнуляет счётчик и фазу текущего упражнения. Пока идёт таймер отдыха после «Стоп», кнопки `Сброс` и `Стоп` неактивны.
...
- **Поведение:** по нажатию «Стоп» сессия останавливается, камера отключается, на пустом canvas показывается обратный отсчёт с кольцом прогресса. Во время этого отсчёта кнопки «Сброс» и «Стоп» отключены до окончания отдыха или нажатия «Старт».
