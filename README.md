...
- **Управление сессией:** одна кнопка переключается между `Старт` и `Пауза`: первый запуск и возобновление после паузы — это `Старт`, во время активного выполнения та же кнопка становится `Пауза`. `Сброс` обнуляет счётчик и фазу текущего упражнения.
