...
7. **Слои главной (`pages/HomePage/logic`, `contexts`, `selectors`, `containers`):** подпапки по соглашению, баррели `index.ts`, импорты между папками `src` — из баррелей. [.cursor/rules/src-layout.mdc](.cursor/rules/src-layout.mdc), [docs/src-layout.md](docs/src-layout.md).
