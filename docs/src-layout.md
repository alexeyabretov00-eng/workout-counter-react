...
- Разметка слота: контейнер рендерит **один** презентационный компонент из **`src/components`** (например панель управления, статус-бар, сцена); стили виджета — в **`<Имя>.css`** рядом с этим компонентом. Глобальные правила и оформление слота `header` при необходимости остаются в `App.css`.
